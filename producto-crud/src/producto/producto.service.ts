...
async findAll(): Promise<Product[]> {
  ...
  async findOne(id: number): Promise<Product> {  // Cambia a 'number'
...
async create(createProductDto: CreateProductDto): Promise<Product> {
  ...
  async update(id: number, updateProductDto: UpdateProductDto): Promise<Product> {  // Cambia a 'number'
    const producto = await this.productRepository.preload({ id, ...updateProductDto });
    if (!producto) throw new NotFoundException('Producto no encontrado');
    return this.productRepository.save(producto);
}

  async remove(id: number): Promise<void> {  // Cambia a 'number'
